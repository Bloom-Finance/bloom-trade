  ...
  async getTransactionHistory(filters: {
    ...
    for (const address of this.addresses) {
      if (
        ...
        //Check if avax is included otherwise use from block 0
        const avaxIncluded = (filters.from as any).selfCustodialProviders.find(
          (e) => e.chain === 'avax'
        );
        startingBlock = avaxIncluded.block;
      }
      const {
      ...
      for (const contract of contracts) {
        const {
          ...
          `${this._baseurl}?module=account&action=tokentx&contractaddress=${
            getAssetDataByChain(contract, this.chain as Chain, this._provider)
              .address
          }&address=${address}&startblock=${startingBlock}&endblock=99999999&page=1&offset=10
